...
const statusBadge: Record<string, { label: string; className: string }> = {
  ...
  draft: { label: "Draft", className: "bg-[#d4a843]/10 text-[#d4a843] border-[#d4a843]/20" },
};
  ...
  return (
    <div className="flex items-end gap-[2px] h-10">
      {data.map((v, i) => (
        <div
          key={i}
          className="bg-[#d4a843]/40 rounded-t-sm min-w-[2px] flex-1"
    ...
    <div className="p-6 space-y-6 overflow-auto h-full" data-testid="page-campaigns">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {campaigns.map((campaign) => (
          <div
            key={campaign.id}
            className="rounded-lg border border-[#1e1e1e] bg-[#111] p-5 hover:border-[#2a2a2a] transition-colors cursor-pointer"
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-emerald-400">{campaign.appointmentsBooked}</p>
                {campaign.qrScans > 0 && (
